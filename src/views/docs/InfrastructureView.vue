<template>
  <div class="docs-page-content">
    ...
    <div class="section-container">
      ...
      <p>
        The probing servers advertise the sub prefixes of <code>2a0e:97c0:8a0::/44</code>, which is dedicated to the nxthdr probing infrastructure. This enables measurements using source unicast and anycast addresses managed by as215011. As with other parts of the network, the limitation is that it is IPv6-only. However, if measurements need to be performed from IPv4-only networks, the server's IPv4 address can possibly be used.
      </p>
    </div>
    ...
    <div class="section-container">
      ...
      <p>
        All essential nxthdr services run on a core server, a <a href="https://www.scaleway.com/en/dedibox/" target="_blank" rel="noopener">Scaleway</a> Dedibox bare-metal server located in Amsterdam. nxthdr relies on various key components, including ClickHouse and PostgreSQL for databases, Redpanda for data streaming, and Prometheus, Loki, and Grafana for observability. These are examples of the technologies currently in use, and they may evolve as the platform grows.
      </p>
      <p>
      ...
      <p>
        The current limitation of as215011 is that it is IPv6-only. To address this, we use an "IPv4 proxy server"--a <a href="https://www.scaleway.com/en/" target="_blank" rel="noopener">Scaleway</a> cloud instance with a public IPv4 address. This server proxies IPv4 traffic to the IPv6-only core server through a <a href="https://caddyserver.com/" target="_blank" rel="noopener">Caddy</a> reverse proxy.
      </p>
      <p>
